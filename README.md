...
본 프로젝트는 **리그 오브 레전드(League of Legends)** 경기 초반(14분 이전) 데이터를 **시계열 모델(LSTM)**로 분석하여,  
각 플레이어의 **라인전 기여도** 및 **초반 주도권 확보 능력**을 객관적으로 평가하는 것을 목표로 합니다.
...
- **경기 흐름 결정:**  
  LOL에서 **14분은 포탑 방패 골드가 소멸**하고, 라인전 단계가 끝나며 본격적인 **오브젝트 및 한타 싸움 단계로 넘어가는 변곡점**입니다.  
  14분까지의 성장과 주도권이 전체 경기의 승패에 큰 영향을 미친다는 것은 **프로 경기 데이터 및 통계적으로 입증된 사실**입니다.

- **라인전 평가 집중:**  
  14분 이전의 데이터만을 모델 학습에 사용함으로써,  
  **중후반 한타나 운영 변수의 영향을 최소화**하고,  
  **순수한 개인 기량과 라인전 수행 능력**에 집중할 수 있습니다.

---

### 1.2  챔피언 특성 뭉개기 (Normalization for Champion Heterogeneity)

챔피언마다 기본 능력치, 스킬셋, 성장곡선, 역할군(예: 캐리형 vs. 탱커형, 초반 강캐 vs. 후반 성장형)이 크게 다르기 때문에  
단순히 **절대 수치(CS 80개, 딜량 5000 등)**로는 공정한 비교가 불가능합니다.  

이를 해결하기 위해 다음 두 가지 **정규화(보정) 전략**을 적용하였습니다.

####   차이 지표 활용 (Difference Metrics)

- `현재 체력 차이(t)`, `경험치 차이(t)` 등은 **상대 플레이어와의 상대적 우위**를 직접적으로 표현합니다.  
- 이는 챔피언의 특성이나 상성과 무관하게, **동일 시간대에 더 우세한 상태**를 수치적으로 보여주는 객관적 지표입니다.  
- 예를 들어, 초반 라인전에서 상대보다 경험치가 높거나 체력이 더 높다면, **라인전 주도권을 확보한 것**으로 간주할 수 있습니다.

####  분당 지표 활용 (Per-Minute Normalization)

- `CS(t)`나 `입힌 피해량(t)` 등 주요 성과 지표를 **분당 단위**로 계산하여,  
  경기 시간의 길이 차이로 인한 왜곡을 제거했습니다.  
- LSTM 모델은 시간에 따른 패턴을 학습하므로,  
  **시간(t) 피쳐와 결합하여 성장 곡선이나 변화율을 학습**할 수 있도록 설계했습니다.  
- 이를 통해 챔피언마다 다른 성장속도나 스파이크 타이밍을 평균화하여 **공정한 비교 기반**을 마련했습니다.

---

### 1.3  KDA (킬/데스/어시스트) 지표의 재해석 및 한계 보완

KDA는 LOL 분석에서 가장 널리 쓰이는 지표이지만,  
이는 본질적으로 **‘결과(Result)’이지 ‘행동(Action)’이 아닙니다.**

####  KDA의 역할

- **KDA는 초반 주도권이 실제 골드 및 스노우볼링으로 이어졌는지를 검증하는 보조 스코어**로 활용됩니다.  
- 즉, 교전에서의 성과가 골드나 오브젝트 이득으로 전이되었는지를 확인하는 **성과 지표(Outcome Indicator)**의 역할을 합니다.

####  KDA의 한계와 보완 전략

- KDA가 낮다고 해서 플레이어의 기여도가 반드시 낮은 것은 아닙니다.  
  예를 들어, 킬이 없더라도 **타워 방패를 파괴하거나 챔피언 외 피해량이 높다면**,  
  이는 **라인전 주도권을 확보하기 위한 적극적인 행동(Action)**을 의미합니다.
- 따라서 본 프로젝트에서는 **KDA 외에 행동 기반 지표(Behavioral Metrics)**를 함께 사용하여  
  “결과만 좋은 플레이”보다 **“주도적으로 경기를 만든 플레이”**를 더 높게 평가하도록 모델을 설계했습니다.

예시:  
> 킬이 없더라도 **타워 방패를 다 뜯고, 상대 체력 차이를 크게 벌린 플레이어**는  
> 모델에서 높은 라인전 평가 점수를 받을 수 있습니다.
...
세 가지 철학을 기반으로, **LSTM 시계열 학습에 최적화된 구조로 설계**되었습니다.
